...
#include "nntoolkitcore/layers/gru1.h"
#include "nntoolkitcore/core/ops.h"
#include "nntoolkitcore/core/memory.h"
#include "stdlib.h"

GRU1Config GRU1ConfigCreate(int input_feature_channels, int output_feature_channels, bool flip_output_gates, bool v2, bool return_sequences, int batchSize, ActivationFunction recurrent_activation, ActivationFunction activation){
    GRU1Config config;
...
GRU1 GRU1CreateForInference(GRU1Config config) {
    GRU1 filter = malloc(sizeof(struct GRU1Struct));
    filter->config = config;
    filter->weights = malloc(sizeof(GRU1Weights));
...
static void GRUCellForward(GRU1 filter, const float *x, const float *h_pr, float* ht, float *buffer) {
